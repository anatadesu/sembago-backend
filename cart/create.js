...
router.post('/addOrder', async (req, res) => {
  try {
    ...
    // Generate automatic ID using Firestore timestamp
    const timestamp = Timestamp.now();
    const orderId = timestamp.seconds.toString(); // Convert timestamp to a string
    ...
    // Save order to Firestore with orderId as document ID
    await orderCollection.doc(orderId).set(order);
