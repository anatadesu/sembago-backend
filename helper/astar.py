...
import heapq
import math
from flask import Flask, request, jsonify  # Menggunakan Flask untuk menerima HTTP request
...
@app.route('/nearProduct', methods=['POST'])
def near_product():
    try:
        # Change: Read input data from HTTP request
        ...
        if isinstance(input_data, dict) and 'user_lat' in input_data and 'user_long' in input_data and 'data' in input_data:
            ...
            return jsonify({'error': 'Invalid input format. Expected a dictionary with keys user_lat, user_long, and data.'})
